...
struct SplinePrimitivesConfig 
{
  ...
  int numEndAngless = 3; //number of possible end orientations per cell (has to be odd)

  /*Primitives will be generated leading to all cells withing this radius (in cells) */
  int destinationCircleRadius = 20;
  /**Offset of the center of a cell from the grid index. */
...
struct SplinePrimitive
{
  ...
  base::Vector2d endPosition;//discret end position, i.e. the cell that this spline ends in
...
{
private:
  std::vector<SplinePrimitive> primitives; //primitives by id
  SplinePrimitivesConfig config;

  
public:
  SbplSplineMotionPrimitives(const SplinePrimitivesConfig& config = SplinePrimitivesConfig());
  
private:
  void generatePrimitives(const SplinePrimitivesConfig& config);
  void generatePrimitivesForAngle(const unsigned startAngle);
//   std::vector<base::Vector2d> calculateDestinationsForAngle(const unsigned startAngle) const;
  std::vector<Eigen::Vector2i> generateDestinationCells(const SplinePrimitivesConfig& config) const;
  std::vector<Eigen::Vector2i> getPixelsOnLine(const int x, const int yStart, const int yEnd)const;
